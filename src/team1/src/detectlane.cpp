...
DetectLane::DetectLane()
{
    ...
    cvCreateTrackbar("LowH", "Threshold Shadow", &minShadowTh[0], 179);
    cvCreateTrackbar("HighH", "Threshold Shadow", &maxShadowTh[0], 179);

    cvCreateTrackbar("LowS", "Threshold Shadow", &minShadowTh[1], 255);
    cvCreateTrackbar("HighS", "Threshold Shadow", &maxShadowTh[1], 255);

    cvCreateTrackbar("LowV", "Threshold Shadow", &minShadowTh[2], 255);
    cvCreateTrackbar("HighV", "Threshold Shadow", &maxShadowTh[2], 255);

    cvCreateTrackbar("LowH1", "Threshold Shadow", &minLaneInShadow[0], 179);
    cvCreateTrackbar("HighH1", "Threshold Shadow", &maxLaneInShadow[0], 179);

    cvCreateTrackbar("LowS1", "Threshold Shadow", &minLaneInShadow[1], 255);
    cvCreateTrackbar("HighS1", "Threshold Shadow", &maxLaneInShadow[1], 255);

    cvCreateTrackbar("LowV1", "Threshold Shadow", &minLaneInShadow[2], 255);
    cvCreateTrackbar("HighV1", "Threshold Shadow", &maxLaneInShadow[2], 255);
...
void DetectLane::update(const Mat &src)
{
    ...
    // circle(lane, leftLane[0], 1, Scalar(0, 255, 0), 2, 8, 0);
    ...
    // circle(lane, rightLane[0], 1, Scalar(0, 255, 0), 2, 8, 0);
...
Mat DetectLane::preProcess(const Mat &src)
{
    ...
    // Mat imgGray, dilatedImg, backgroundImg, diffImg, cloneImg, normalizedImg, revertedDiffImg, thresholdedImg, finalImg;

    // // convert image to grayscale
    // cvtColor(src, imgGray, CV_RGB2GRAY);

    // // dilate the image to get rid of details
    // dilate(imgGray, dilatedImg, getStructuringElement(MORPH_ELLIPSE, Size(7, 7)));

    // // blur the image to better remove details and contain all the shadows
    // medianBlur(dilatedImg, backgroundImg, 21);

    // // calculate the distance between the original and shadow image
    // absdiff(imgGray, backgroundImg, diffImg);

    // diffImg.copyTo(cloneImg);

    // normalize(cloneImg, normalizedImg, 0, 255, NORM_MINMAX, CV_8UC1);

    // threshold(normalizedImg, thresholdedImg, 230, 0, THRESH_TRUNC);

    // normalize(thresholdedImg, finalImg, 0, 255, NORM_MINMAX, CV_8UC1);

    // Mat imgBGR;

    // cvtColor(finalImg, imgBGR, COLOR_GRAY2BGR);

    // cvtColor(imgBGR, imgHSV, COLOR_BGR2HSV);

    // imshow("No shadow", imgHSV);
    ...
    try
    {
        imgHSVShadow = laneInShadow(src);

        imshow("Shadow", imgHSVShadow);
    }
    catch (...)
    {
        ROS_ERROR("Cannot find lane in shadow");
        throw;
    }
    ...
    // combinedShadowAndNoShadow(imgHSVShadow, imgThresholded);

    Mat imgMerged = imgThresholded + imgHSVShadow;

    Mat imgMergedMorph, imgBirdMorph;

    imshow("Binary", imgMerged);

    imgMergedMorph = morphological(imgMerged);
    imshow("Morphological Image Merged", imgMergedMorph);

    dst = birdViewTranform(imgMerged);

    imshow("Bird View", dst);

    fillLane(dst);

    imgBirdMorph = morphological(dst);
    imshow("Morphological Image BirdView", imgBirdMorph);

    // try{
    // combinedShadowAndNoShadow(imgHSVShadow, dst);}
    // catch(...){
    //     ROS_ERROR("Cannot combine");
    // }
...
Mat DetectLane::laneInShadow(const Mat &src)
{
    ...
    {
        ROS_ERROR("Cannot convert from BGR to HSV");
    }
...
Mat DetectLane::combinedShadowAndNoShadow(const Mat &src, Mat &dest)
{
    const int row = src.rows;
    const int column = src.cols;

    for (int i = 0; i < row; ++i)
    {
        for (int j = 0; j < column; ++j)
        {
            if (src.at<char>(i, j) == 255)
            {
                dest.at<char>(i, j) = 255;
            }
        }
    }
}